...
- **GTX API** is free but rate-limited. Use a paid API for long-running jobs — see [GTX Free essentials](#gtx-free-essentials) below.
...
GTX is Google Translate's free public endpoint — zero-config, works out of the box, and the tool's **default service**. Requests go **directly from your browser** to `translate.googleapis.com`; your text never touches this tool's servers. The flip side: availability depends on **your network's path to Google** — the same moment working for one user and failing for another is normal for a shared free endpoint, not a bug in the tool.

### Rate limits and automatic slowdown

The shared free endpoint is rate-limited per user. Heavy bursts (large files, raised concurrency, multi-file batches) can trigger throttling. The tool handles this automatically:
...
- If the failure panel keeps showing rate-limit messages: wait a few minutes and hit "Retry" (the cache skips completed lines), lower **concurrency** in Advanced Settings, or switch long batch jobs to a keyed service like DeepL / Qwen-MT / DeepSeek

Can't connect, or seeing CORS errors in the console? It's almost always a network-environment issue (mainland China blocking, corporate network interception, Google abuse protection, or browser extensions) — see [FAQ → GTX Free cannot connect](./faq.mdx#gtx-free-cannot-connect-or-shows-cors-errors) for the checklist.
