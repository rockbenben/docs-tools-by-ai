...
description: Tools By AI integrates 8 translation APIs and 21 LLMs. This page helps you pick the right one and walks through local model setup and CORS configuration.
---
...
| Chinese ↔ English, reliability | **DeepSeek** / **Qwen-MT Plus** / **Claude** | Works in mainland CN, great CN/EN quality |
...
| GTX Web (Free)   | ★★★☆☆   | ★★☆☆☆     | Small-volume translation          | Free                                     |
...
- **Thinking mode**: lets the AI think before translating — higher quality, slower and pricier. Supported models show a toggle in the UI. The toggle is **stored per model** so switching models preserves each one's setting independently. The exact UI form depends on the provider:
  - **Three levels** (off / low / medium / high): Claude, Gemini, DeepSeek, OpenAI GPT-5, Qwen3, Azure OpenAI, Nvidia NIM
  - **Binary toggle** (off / on): Doubao, Zhipu GLM, Moonshot (Kimi), MiniMax, Hunyuan TurboS (underlying APIs accept on/off but not an effort param)
  - **Low / high only**: xAI Grok 4.3 (API limitation)
  - **Always-on SKUs** (no toggle shown): ERNIE 5.0-Thinking, Hunyuan 2.0-Thinking, Mistral Magistral, Perplexity Sonar Reasoning Pro / Deep Research, Cohere Command A Reasoning, Grok 4.20 Reasoning / Multi-Agent
...
- **Limited language coverage**: only ~55 mainstream languages (Google's WMT24++ benchmark scope). Low-resource languages (Cantonese (yue), Bhojpuri (bho), Wolof (wo), Aragonese (an), Guarani (gn), Kurdish (ckb/kmr), and ~62 others) are blocked by the UI. Use DeepL / Google / Azure / Qwen-MT for broader coverage
