...
faq:
  - q: What if the translation result is empty, shows only the original text, or returns null?
    a: >-
      If only a few lines show the original text, that's the failed-line fallback — click "Retry failed lines"
  ...
  - q: Will failed translations retry automatically? What's not retried?
    a: >-
      ...
      panel with one-click Retry failed lines. Errors where retrying can't help need a settings change
  ...
  - q: Translated subtitles are out of order, or several lines got merged into one — what to do?
    a: >-
      ...
      (e.g. 3 → 1), and use the red panel's Retry failed lines to fill the gaps.
  - q: How do I use AI translation for free, without a paid API key?
    a: >-
      ...
      Nvidia NIM, OpenCode Zen (the `(free)` SKUs), Cerebras (1M tokens/day), SiliconFlow, and more. Note that
  ...
  - q: Running translategemma on Ollama is slow, dropping lines, or translating badly — what's wrong?
    a: >-
      Two causes, both worth fixing. (1) Don't run TranslateGemma / MiLMMT on Ollama: they
      pre-render the prompt and POST to /v1/completions, but Ollama still applies the Modelfile
      template there (verifiable in its source), wrapping the prompt a second time — quality
      degrades for no visible reason and nothing errors. Use LM Studio / llama.cpp / koboldcpp.
      (2) Don't configure it as a regular LLM ("Custom (OpenAI-compatible)" with
  ...
  - q: Why does DeepL go through a relay server?
    a: >-
      ...
      so the tool routes requests through a built-in relay — same for Nvidia NIM. OpenCode Zen, TokenHub and
  ...
  - q: Is there a CLI, API, or desktop version?
    a: >-
      There is a CLI. The translation engine is platform-agnostic TypeScript, and the repo ships
      a command-line entry point (`yarn cli`) that reuses the exact same pipeline as the web app —
      batching, concurrency, retries, 429 cooldown, glossary, context translation and per-line
      caching all behave identically. It handles subtitles, Markdown and JSON, and can read the
      settings JSON exported from the web UI. See [Command-line translation](./cli.mdx). There is
      no hosted API and no downloadable desktop build; on the web, batch mode, Import settings and
      pasted language codes (zh, de, ru) cut down the manual steps.
---
...
> Related: frequent **timeouts** on these two are usually not the model but concurrency exceeding your server's parallel slots — see [Local Model Setup](./api.mdx#local-model-setup).
