...
description: Explore advanced features like batch translation, caching, and configuring API parameters like chunk size and delay to build an efficient and stable translation workflow.
---
...
- **Example**: Enter `♪ ♫ 🎵` to remove music symbols from subtitles
- **Use Cases**:
  - Remove music markers from subtitles (♪, ♫, 🎵, etc.)
  - Remove HTML tags (such as `</i>`, `<b>`)
...
"Translation Rate" corresponds to the **Translation Speed / limit** parameter in the tool, representing the **number of concurrent requests sent to the API**. A higher value yields greater theoretical throughput, but it also increases the likelihood of being rate-limited or receiving errors such as HTTP 429.

If frequent errors occur, it is recommended to **lower the rate or increase the delay**, for example, by reducing it to 20 or lower first, and then adjusting it back up once stability is confirmed.
