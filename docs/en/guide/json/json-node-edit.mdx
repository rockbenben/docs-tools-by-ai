...
howto:
  ...
  steps:
    ...
    - name: Pick an operation
      text: Add Prefix / Add Suffix / Find Text / Replace Node Value — pick what you need.
...
faq:
  ...
  - q: Can I combine the three edit operations (prefix/suffix, find/replace, direct replace)?
    a: >-
      Prefix/suffix and find/replace stack together. But "Replace Node Value" is exclusive —
  ...
  - q: How do I auto-replace text with the matching language name in multilingual JSON?
    a: >-
      Toggle "Language code → name". The tool reads the language code in the node path (zh, en,
      ja, ko, es, fr, de, it, ru, pt, hi, ar, bn — 13 codes supported) and replaces the contents of
      "Find Text" with the corresponding language name (`en` → `English`), so you don't have to configure
      13 separate find/replace rules. Two conditions: the code must be a **whole path segment**
      (`$['hi']['title']` matches `hi`, while the `es` inside `description` is not mistaken for Spanish),
      and the toggle does nothing while "Find Text" is empty.
  - q: Will it lag on very large JSON files?
    a: >-
      For big files, drag them into the upload area instead of pasting into the text box — noticeably
      smoother. All processing runs locally in your browser — files are never uploaded, and tens of
      megabytes work fine.
