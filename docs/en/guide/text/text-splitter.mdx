...
faq:
  ...
  - q: Why is the default character count 5000?
    a: Because the remaining hard limit measured in characters comes from machine-translation APIs (both Google Translate and DeepL cap the free tier at 5000 characters per request), not from LLM context — GPT-5.6 is around 400K tokens and Claude / Gemini / DeepSeek are all at 1M, so 2000-character chunks accomplish nothing. Leaving the field empty falls back to 5000.
