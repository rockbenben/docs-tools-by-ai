...
description: Web novel layout messy and full of ads? One-click fix for line breaks, chapter titles, Trad/Simp conversion, ad removal — plus chapter reorder and custom replacement rules. Batch processing, runs locally in your browser.
image: https://docs.newzone.top/img/novel-processor-en.webp
howto:
...
faq:
  ...
  - q: How do "Content Filter" and "Protect long lines" work together?
    a: Content Filter accepts comma-separated keywords (e.g. `channel,downloader,group`) — every line containing any of them is removed. To avoid wiping legitimate text, set Filter Threshold to N → lines longer than N characters are exempt (kept even if they match a keyword). 0 disables the exemption. This lets you clean short watermark lines while preserving long body paragraphs.
  - q: What's the "Custom Replacement Rules" and is it related to the Chinese Converter?
    a: Yes — both tools share the same custom replacement rules (same localStorage key). It defines terms that should NOT be converted during Trad/Simp transformation. It only applies when Trad/Simp conversion is actually enabled in Advanced Settings (set to t2s or s2t). When set to "None", the panel shows a "rules inactive" hint.
...
For deeply-obfuscated watermarks (scrambled-character promotional content), combine **Content Filter** with **Protect long lines** as a first pass. Extremely obfuscated patterns are best handled by the [Text Toolbox](./text-toolbox.mdx) with a custom regex, then fed back here for layout.
