...
howto:
  name: How to clean up web novels with Novel Processor
  steps:
    - name: Import the novel TXT
      text: Paste text or upload a TXT file (no size limit — handles 3M-character mega-novels fine).
    - name: Configure repair rules
      text: Toggle the rules you need — smart line break, chapter split, paragraph indent, dedupe lines, Trad/Simp conversion, junk filter. Rules are independent and stackable.
    - name: Export reader-ready TXT
      text: Export the cleaned TXT and send via Kindle push email, or import into WeChat Read, iReader, Moon Reader. Chapter Split enables Kindle's auto-generated TOC.
faq:
...
For deeply-obfuscated watermarks (scrambled-character promotional content), combine **Filter Words** with **Filter Threshold** as a first pass. Extremely obfuscated patterns are best handled by the [Text Toolbox](./text-toolbox.mdx) with a custom regex, then fed back here for layout.
