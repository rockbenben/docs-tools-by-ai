...
howto:
  ...
  steps:
    ...
    - name: Toggle the ignore options you need
      text: Ignore surrounding whitespace, ignore case, and character-level diff can be combined freely.
...
faq:
  ...
  - q: What if the two texts differ so much the comparison times out?
    a: A 3-second timeout guard stops the run and shows a notice instead of freezing the page. If you really need the result, click "Compare anyway" to force a full comparison (which may be slow). You can also switch to "Only differences" or reduce the context lines to focus the view.
  - q: How do I ignore whitespace or case differences?
    a: Tick "Ignore surrounding whitespace" to skip differences caused by edge spacing, and "Ignore case" to skip letter-case differences. Both can be on at once — handy for comparing code or config files that were reformatted.
