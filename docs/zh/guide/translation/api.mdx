...
description: 工具集成了 9 种机器翻译 API 与 26 种大模型接口，本页帮你快速选出最合适的接口，并提供本地模型接入与跨域配置指南。
---
...
| MiLMMT-46        | ★★★★☆ | ★★★★☆  | 本地翻译，47 语言（含粤语）    | 自托管免费                 |
...
- **模型**：可以下拉选，也可以直接输入。下拉里每条显示“友好名 + 真实 SKU”两行，该接口的默认模型带 `default` 标签；厂商刚发布、清单里还没有的 SKU 手填即可。Azure OpenAI 填部署名。
