...
本工具集成了 7 种翻译 API 和 17 种主流大语言模型（LLM）接口，用户可根据需求选择合适的翻译方式：
...
| Qwen-MT          | ★★★★★    | ★★★★★  | 支持特定领域(Domain)翻译     | 按 Token 计费，新用户享免费额度                |
...
如果对翻译速度和质量有更高要求，可自行申请 API Key。申请流程参考相关的[接口申请教程](https://ttime.timerecord.cn/service/translate/google.html)。

## LLM 模型翻译

除传统翻译 API 外，本工具还支持调用多种 LLM 进行智能翻译，包括：DeepSeek、OpenAI、Claude、Gemini、Qwen、Moonshot (Kimi)、Zhipu GLM、Doubao（火山方舟）、xAI (Grok)、Mistral、Perplexity、OpenRouter、Groq、SiliconFlow、Nvidia NIM、Azure OpenAI，以及可自由配置的 Custom LLM。
...
为了解决浏览器直接调用官方 API 时可能遇到的跨域（CORS）问题，**DeepL** 和 **Nvidia** 默认使用内置的代理服务。
