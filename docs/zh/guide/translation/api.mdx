...
支持的 LLM 接口：DeepSeek、OpenAI、Claude、Gemini、Qwen、Moonshot (Kimi)、Zhipu GLM、Doubao、Xiaomi MiMo、MiniMax、Tencent Hunyuan、Baidu ERNIE、Cohere、xAI (Grok)、Mistral、Perplexity、OpenRouter、Groq、SiliconFlow、GitHub Models、Nvidia NIM、Azure OpenAI，以及兼容 OpenAI 协议的自定义接口。
...
- **思考模式**：让 AI 多想一步再翻译，质量更高但更慢/更贵——支持的模型 UI 会显示开关。开关**按模型独立保存**，切模型不影响别的设置。具体 UI 形态因 provider 而异：
  ...
  - **无思考开关**：GitHub Models（网关不支持 reasoning 参数）
