...
| 中英互译、要稳定 | **DeepSeek** / **Qwen-MT Plus** / **Claude** | 国内可用、中英效果好 |
...
- **思考模式**：让 AI 多想一步再翻译，质量更高但更慢/更贵——支持的模型 UI 会显示开关。开关**按模型独立保存**，切模型不影响别的设置。具体 UI 形态因 provider 而异：
  - **三档调节**（off / low / medium / high）：Claude、Gemini、DeepSeek、OpenAI GPT-5、Qwen3、Azure OpenAI、Nvidia NIM
  - **两档开关**（off / on）：Doubao、Zhipu GLM、Moonshot (Kimi)、MiniMax、Hunyuan TurboS（底层 API 只接受 on/off，不接受 effort）
  - **仅 low / high**：xAI Grok 4.3（API 限制）
  - **选了即开启**（UI 不显示 toggle）：ERNIE 5.0-Thinking、Hunyuan 2.0-Thinking、Mistral Magistral、Perplexity Sonar Reasoning Pro / Deep Research、Cohere Command A Reasoning、Grok 4.20 Reasoning / Multi-Agent
...
- **语言覆盖较小**：只支持约 55 种主流语言（对齐 Google WMT24++ benchmark）；小语种（粤语 yue、博杰普尔 bho、沃洛夫 wo、阿拉贡 an、瓜拉尼 gn、库尔德 ckb/kmr 等约 68 种）会被 UI 直接拦截。需要更广语种请改用 DeepL / Google / Azure / Qwen-MT
